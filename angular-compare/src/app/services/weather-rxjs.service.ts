...
import { map, switchMap, catchError, startWith, shareReplay } from 'rxjs/operators';
...
export class WeatherRxJSService {
  ...
  private searchSubject = new BehaviorSubject<{ lat: number; lon: number } | null>(null);
  ...
  readonly weatherData$: Observable<WeatherRxJSState> = this.searchSubject.pipe(
    switchMap(coords => {
      ...
      return this.http.get<LocationPoint>(`https://api.weather.gov/points/${coords.lat},${coords.lon}`).pipe(
        switchMap(point => {
